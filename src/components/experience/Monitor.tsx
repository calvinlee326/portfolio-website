  ...
  return (
    <group ref={group}>
      ...
      <RoundedBox args={[3.5, 2.55, 0.35]} radius={0.12} smoothness={4} position={[0, 0, 0]}>
        <meshStandardMaterial color="#11151f" metalness={0.4} roughness={0.45} />
      </RoundedBox>

      {/* Screen — emissive so it glows in the dark */}
      <mesh position={[0, 0.08, 0.18]}>
        <planeGeometry args={[3.0, 1.95]} />
        <meshStandardMaterial color="#05080f" emissive="#0b2a3a" emissiveIntensity={0.6} roughness={0.3} />
      </mesh>
      ...
      <group position={[-1.35, 0.78, 0.2]}>
        ...
        <Text position={[0, -0.62, 0]} fontSize={0.15} color="#e2e8f0" anchorX="left" anchorY="top">{`user: ${NAME}`}</Text>
      ...
      <mesh position={[0, -1.55, -0.1]}>
        ...
        <meshStandardMaterial color="#11151f" metalness={0.4} roughness={0.5} />
      </mesh>
      <mesh position={[0, -1.85, -0.1]}>
        <boxGeometry args={[1.1, 0.12, 0.6]} />
        <meshStandardMaterial color="#0d1019" metalness={0.4} roughness={0.5} />
      </mesh>
    </group>
  )
}
